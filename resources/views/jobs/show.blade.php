            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-8">
                    <div class="flex flex-wrap gap-2 mb-5">
                        ...
                        <span class="text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full
                            {{ $job->status === 'open' ? 'bg-[#c8f135]/30 text-gray-700' : 'bg-red-50 text-red-500' }}">
                            {{ $job->status === 'open' ? '● Open' : '● Gesloten' }}
                        </span>
                    </div>

                    <h1 class="font-black text-3xl tracking-tight leading-snug mb-2">{{ $job->title }}</h1>
                    <p class="text-sm text-gray-400">
                        Geplaatst door <span class="font-semibold text-gray-600">{{ $companyName }}</span>
                        · {{ $job->created_at->format('d M Y') }}
                        · <span class="font-medium">{{ $job->applications->count() }} {{ $job->applications->count() === 1 ? 'reactie' : 'reacties' }}</span>
                    </p>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-8">
                    <h3 class="font-black text-lg mb-5 flex items-center gap-2">
                        <span class="w-1 h-5 bg-[#c8f135] rounded-full inline-block"></span>
                        Beschrijving
                    </h3>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-8">
                    <h3 class="font-black text-lg mb-5 flex items-center gap-2">
                        <span class="w-1 h-5 bg-[#0a0a0a] rounded-full inline-block"></span>
                        Vereisten
                    </h3>
                    <div class="text-gray-600 leading-relaxed whitespace-pre-line text-sm">{{ $job->requirements }}</div>
                </div>
                @endif
            ...
            <div class="space-y-5 lg:sticky lg:top-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-6">
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-12 h-12 bg-[#0a0a0a] text-white rounded-xl flex items-center justify-center font-black text-lg flex-shrink-0">
                ...
                @else
                    <div class="hidden lg:block bg-white rounded-2xl border border-gray-200 p-6">
                        ...
                        <div class="flex flex-col gap-2">
                            <a href="{{ route('login') }}"
                                class="text-center bg-[#0a0a0a] text-white font-bold px-5 py-2.5 rounded-full hover:bg-gray-800 transition text-sm">
                                Inloggen
                            </a>
                            <a href="{{ route('register') }}"
                                class="text-center border border-gray-300 text-gray-600 font-semibold px-5 py-2.5 rounded-full hover:border-gray-900 hover:text-gray-900 transition text-sm">
                                Account aanmaken
                            </a>
                        </div>
                    </div>
