<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl tracking-tight">Opdrachten</h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto px-6 py-10">
        ...
        <form method="GET" class="bg-white rounded-2xl border border-gray-200 p-5 mb-10 flex flex-wrap gap-3 items-end">
            ...
            @if(request('search') || request('region') || request('type'))
                <a href="{{ route('assignments.index') }}" class="text-sm text-gray-400 hover:text-gray-700 self-center">Wis filters</a>
            @endif
        </form>

        @if($assignments->isEmpty())
            <div class="text-center py-24 text-gray-400">
                <p class="text-lg font-medium">Geen opdrachten gevonden.</p>
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <a href="{{ route('assignments.show', $assignment) }}"
                    ...
                    <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                        <div>
                            <p class="text-sm font-semibold text-gray-800">{{ $assignment->company->companyProfile->company_name ?? $assignment->company->name }}</p>
                            <p class="text-xs text-gray-400 mt-0.5">{{ $assignment->created_at->diffForHumans() }}</p>
                        </div>
            ...
            <div class="mt-8">{{ $assignments->withQueryString()->links() }}</div>
        @endif
    </div>
</x-app-layout>
